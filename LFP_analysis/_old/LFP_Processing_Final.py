...
#Dictate whether EMG electrodes are present (based on experimental configuration) 
#and allocate file names accordingly
...
#If all channels are used for cell recording, move on.
if noncell_channels == 0:
	fieldValues = []
	
#If there are channels used, otherwise: create variable array for channel specification
else:
    # <<< Doesn't this just generate a list starting at 0?
    # <<< What if noncell channels are in the middle somewhere?
	Fields = []
	for channel in range(noncell_channels):
		chan_variable = "Channel " + "%01d" % (channel)
		Fields.append(chan_variable)
        #Specify which channels are used for non-cell recording
	fieldValues = easygui.multenterbox('Which Channels are these (i.e. EMG)?', 
                                            'Used Channels?', 
                                           ['{}'.format(i) for i in range) 
        #Check to make sure that user input has as many non-cell 
        #channel numbers as they indicated were used
        #If user messed up, they will be given a chance to correct this
	for i in range(len(Fields)):
		if fieldValues[i-1].strip() == "":
			...
			fieldValues = easygui.multenterbox(
                            'Your input did not match number of entries. ' \  
                            + 'Pay Attention!', 'Used Channels?', Fields)

EMG_Channels = fieldValues
#Create and fill array with appropriate .dat file names 
#according to occupied cell recording channels
Raw_Electrodefiles = []
...
freqparam = easygui.multenterbox(
    'Specify LFP bandpass filtering paramter (assumes 30kHz as sampling rate)', 
...
def get_filtered_electrode(data, freq = freqparam, sampling_rate = 30000.0):
	...
	m, n = butter(
            2, 
            [2.0*int(freqparam[0])/sampling_rate, 2.0*int(freqparam[1])/sampling_rate], 
            btype = 'bandpass')
