...
msg   = "Do you want to perform LFP analyses for more than ONE trial (ie. Do you have several tastes) ?"
...
if trial_check == "No":
    analysis_params = easygui.multenterbox(msg = 'Input analysis paramters:', fields = ['Taste array start time (ms)', 'Taste array end time (ms)', 'Sampling Rate (samples per second)', 'Signal Window (ms)', 'Window Overlap (ms; default 90%)'], values = ['0','1200000','1000','1000','900'])
    ...
    plotting_params = easygui.multenterbox(msg = 'Input plotting paramters:', fields = ['Minimum frequency (Hz):','Maximum frequency (Hz):'], values = ['3','40'])
    ...
    # Ask if this analysis is an average of normalization (not supported for passive sitting yet)
    msg   = "Do you want to normalize to baseline (time before stimulus) or have a mean (post-stimulus)?"
    ...
    if analysis_type == "Normalize":
        analysis_params_2 = easygui.multenterbox(msg = 'Input normalizing paramter:', fields = ['Baseline (pre-stimulus signal duration (ms))'], values = ['2000'])
...
print('analysis will use signals: ' + str(base_time) + 'ms' + ' before stim onset & ' + str(upper) + 'ms after stim onset')
...
for taste in range(len(LFP_data)):
	...
	#Create figure
	fig,axes = plt.subplots(nrows=np.size(channel_data,axis=1), ncols=1,sharex=True, sharey=False,figsize=(12, 8), squeeze=False)
	...
	fig.suptitle('%s - Channel Check: %s' %(taste_params[taste], hdf5_name[0:4])+'\n' + 'Raw LFP Traces; Date: %s' %(re.findall(r'_(\d{6})', hdf5_name)[0]),size=16,fontweight='bold')
	fig.savefig('./LFP_channel_check/' + hdf5_name[0:4] + '_ %s_%s' %(re.findall(r'_(\d{6})', hdf5_name)[0],taste_params[taste]) + '_channelcheck.png')   
	plt.show()
	plt.close(fig)
