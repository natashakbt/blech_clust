...
for dir_name in dirs:
	...
	for band in range(len(LFP_data_sliced)):
		...
		for channel in range(np.array(LFP_data_sliced[band]).shape[1]):
			...
			bins = 	np.percentile(np.array(LFP_data_sliced[band])[:,channel,2,:].real,np.linspace(0,100,100))
			
			power_hist = []
			for trial in range(len(np.array(LFP_data_sliced[band])[:,channel,2,:].real)):
				power_hist.append(np.histogram(np.array(LFP_data_sliced[band])\
			...
			#Save plots
			fig.savefig(save_name+'/%s'  %(iter_freqs[band][0])+'/Channel_%i_Power.png' %(channel))   
			plt.close(fig)	
