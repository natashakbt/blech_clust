...
# =============================================================================
	...
	#Save output
	fig.savefig(dir_name+'/%s_%s_Full_Session_raster.png' %(hdf5_name[:4],hdf5_name.split('_')[2].split('_')[0]))   
	plt.close(fig)	
	...
	#Save output
	fig.savefig(dir_name+'/%s_%s_Full_Session_FRs.png' %(hdf5_name[:4],hdf5_name.split('_')[2].split('_')[0]))   
	plt.close(fig)	
	...
	#Save output
	fig.savefig(dir_name+'/%s_%s_Full_Session_ZFRs.png' %(hdf5_name[:4],hdf5_name.split('_')[2].split('_')[0]))   
	plt.close(fig)	
	...
	if "/Whole_session_raw_LFP/WS_LFP_filtered" in hf5:
		...
		for band in trange(len(iter_freqs), desc = 'bands'):
			...
			hf5.create_array('/Whole_session_raw_LFP','WS_LFP_filtered_band_%i' %(band),np.asarray(output))	
			hf5.flush()
