...
# Ask the user which digital input channels should be used for slicing 
# out EMG arrays, and convert the channel numbers into integers for 
# pulling stuff out of change_points
...
durations = easygui.multenterbox(
        msg = 'What are the signal durations pre and post stimulus '
...
for i in range(len(emg_pathname)):
    ...
    for j in range(len(dig_in_channels)):
        for k in range(len(change_points[dig_in_channel_nums[j]])):
            raw_emg_data = data[change_points[dig_in_channel_nums[j]][k]\
                    -durations[0]*30:change_points[dig_in_channel_nums[j]][k]\
                    +durations[1]*30]
