...
def laser_off_trials(data, num_emissions):
	...
	with pm.Model() as model:
		...
		t1 = pm.DiscreteUniform('t1', lower = 10, upper = 50)
		# Switch from identity to palatability firing
		t2 = pm.DiscreteUniform('t2', lower = t1 + 20, upper = 150)
		# Switch from palatability firing to end
		t3 = pm.DiscreteUniform('t3', lower = t2 + 30, upper = 230)
		...
		# Get the actual state numbers based on the switch times
		states1 = tt.switch(t1 >= np.arange(250), 0, 1)
		states2 = tt.switch(t2 >= np.arange(250), states1, 2)
		states = tt.switch(t3 >= np.arange(250), states2, 3)
	...
	# Inference button :D
	with model:
		tr = pm.sample(300000, init = None, step = pm.Metropolis(), njobs = 2, start = {'t1': 20, 't2': 70, 't3': 120}, progressbar = False)
...
def laser_early_trials(data, num_emissions):
	...
	with pm.Model() as model:
		...
		t1 = pm.DiscreteUniform('t1', lower = 10, upper = 50)
		# Switch from identity to palatability firing
		t2 = pm.DiscreteUniform('t2', lower = t1 + 20, upper = 140)
		# Switch from palatability firing to end
		t3 = pm.DiscreteUniform('t3', lower = t2 + 30, upper = 180)
...
def laser_middle_trials(data, num_emissions):
	...
	with pm.Model() as model:
		...
		# Switch from palatability firing to end
		t3 = pm.DiscreteUniform('t3', lower = t2 + 20, upper = 180)
	...
	# Inference button :D
	with model:
		tr = pm.sample(300000, init = None, step = pm.Metropolis(), njobs = 2, start = {'t1': 20, 't2': 60, 't3': 110}, progressbar = False)
