...
for n_states in range(min_states, max_states + 1):
	result = poisson_hmm_implement(n_states, threshold, seeds, n_cpu, binned_spikes, on_trials, edge_inertia, dist_inertia)
	hmm_results.append((n_states, result))

# Delete the poisson_hmm_results node under /spike_trains/dig_in_(taste)/ if it exists
try:
	hf5.remove_node('/spike_trains/dig_in_%i/poisson_hmm_results' % taste, recursive = True)
except:
	pass

# Then create the poisson_hmm_results group
hf5.create_group('/spike_trains/dig_in_%i/' % taste, 'poisson_hmm_results')
hf5.flush()

# Delete the Poisson folder within HMM_plots if it exists for this taste
try:
	os.system("rm -r ./HMM_plots/dig_in_%i/Poisson" % taste)
except:
	pass	

# Make a folder for plots of Multinomial HMM analysis
os.mkdir("HMM_plots/dig_in_%i/Poisson" % taste)
...
for result in hmm_results:
	...
	os.mkdir("HMM_plots/dig_in_%i/Poisson/states_%i" % (taste, result[0]))
		
	# Make a group under poisson_hmm_results for this number of states
	hf5.create_group('/spike_trains/dig_in_%i/poisson_hmm_results' % taste, 'states_%i' % (result[0])) 
	# Write the emission and transition probabilties to this group
	emission_probs = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/states_%i' % (taste, result[0]), 'emission_probs', result[1][4])
	transition_probs = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/states_%i' % (taste, result[0]), 'transition_probs', result[1][5])
	posterior_proba = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/states_%i' % (taste, result[0]), 'posterior_proba', result[1][6])

	# Also write the json model string to file
	model_json = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/states_%i' % (taste, result[0]), 'model_json', result[1][0])

	# Write the log-likelihood, AIC/BIC score, and time vector to the hdf5 file too
	log_prob = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/states_%i' % (taste, result[0]), 'log_likelihood', np.array(result[1][1]))
	aic = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/states_%i' % (taste, result[0]), 'aic', np.array(result[1][2]))
	bic = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/states_%i' % (taste, result[0]), 'bic', np.array(result[1][3]))
...
if laser_exists:
	...
	try:
		exec("hf5.remove_node('/spike_trains/dig_in_%i/poisson_hmm_results/laser' % taste, recursive = True)")
	except:
		pass

	# Then create the poisson_hmm_results group
	exec("hf5.create_group('/spike_trains/dig_in_%i/poisson_hmm_results' % taste, 'laser')")
	hf5.flush()

	# Delete the laser folder within HMM_plots/Poisson if it exists for this taste
	try:
		os.system("rm -r ./HMM_plots/dig_in_%i/Poisson/laser" % taste)
	except:
		pass	

	# Make a folder for plots of Poisson HMM analysis
	os.mkdir("HMM_plots/dig_in_%i/Poisson/laser" % taste)
	
	# Go through the HMM results, and make plots for each state and each trial
	for result in hmm_results:
		
		# Make a directory for this number of states
		os.mkdir("HMM_plots/dig_in_%i/Poisson/laser/states_%i" % (taste, result[0]))
		
		# Make a group under poisson_hmm_results for this number of states
		hf5.create_group('/spike_trains/dig_in_%i/poisson_hmm_results/laser' % taste, 'states_%i' % (result[0])) 
		# Write the emission and transition probabilties to this group
		emission_probs = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/laser/states_%i' % (taste, result[0]), 'emission_probs', result[1][4])
		transition_probs = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/laser/states_%i' % (taste, result[0]), 'transition_probs', result[1][5])
		posterior_proba = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/laser/states_%i' % (taste, result[0]), 'posterior_proba', result[1][6])
		...
		log_prob = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/laser/states_%i' % (taste, result[0]), 'log_likelihood', np.array(result[1][1]))
		aic = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/laser/states_%i' % (taste, result[0]), 'aic', np.array(result[1][2]))
		bic = hf5.create_array('/spike_trains/dig_in_%i/poisson_hmm_results/laser/states_%i' % (taste, result[0]), 'bic', np.array(result[1][3]))
