...
check_str = f'Digital input channels: {dig_in_pathname} \n'\
...
#check = easygui.ynbox(msg = check_str, i
...
if continue_bool:
    ...
    dig_in_channel_nums = [int(x) for x in chosen_digin]
    # If 111, select all
    if 111 in dig_in_channel_nums:
        dig_in_channel_nums = np.arange(len(dig_in_pathname))    
...
json_name = hdf5_name.split('.')[0] + ".params"
with open(params_file,'r') as params_file_connect:
    params_dict = json.load(params_file_connect)
durations = [2000, 5000]
params_dict.append({'psth_durations':durations})

with open(json_name, 'w') as params_file:
    json.dump(params_dict, params_file, indent = 4)
            ...
            # Otherwise run through the units and convert their spike times to milliseconds
                    ...
                    for k in range(len(units)):
                            ...
                            spike_times = np.where(\
                                    (units[k].times[:] <= end_points[dig_in_channel_nums[i]][j]\
                                    + durations[1]*30)*(units[k].times[:] >= \
                                    end_points[dig_in_channel_nums[i]][j] - durations[0]*30))[0]
                            ...
                            #        end_points[dig_in_channel_nums[i]][j] - \
            ...
            for j in range(len(end_points[dig_in_channel_nums[i]])):
                    ...
                    for laser in range(len(laser_nums)):
                            on_trial = np.where(\
                                    ...
                                    end_points[dig_in_channel_nums[i]][j]) <= 5*30000)[0]
                            if len(on_trial) > 0:
